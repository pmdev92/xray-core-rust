...
for target in "${IOS_ARCHS[@]}" "${MACOS_ARCHS[@]}"; do
    ...
    cp "target/$target/release/lib${CRATE_NAME}.a" "$OUTPUT_DIR/$target/"
done
...
xcodebuild -create-xcframework \
    -library "$OUTPUT_DIR/aarch64-apple-ios/lib${CRATE_NAME}.a" -headers "$HEADERS_DIR" \
    -library "$OUTPUT_DIR/ios-simulator-universal/lib${CRATE_NAME}.a" -headers "$HEADERS_DIR" \
    -library "$OUTPUT_DIR/macos-universal/lib${CRATE_NAME}.a" -headers "$HEADERS_DIR" \
