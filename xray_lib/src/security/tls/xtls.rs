...
pub struct TlsXtlsSecurityStream {
...
impl AsyncRead for TlsXtlsSecurityStream {
    fn poll_read(
...
impl AsyncWrite for TlsXtlsSecurityStream {
    fn poll_write(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &[u8],
...
struct XtlsSyncAdapter<'a, 'b> {
    pub stream: &'a mut Box<dyn AsyncXrayTcpStream>,
...
impl XtlsSyncAdapter<'_, '_> {
    ...
    fn poll_with<U>(
        &mut self,
        f: impl FnOnce(
            Pin<&mut Box<dyn AsyncXrayTcpStream>>,
            &mut Context<'_>,
