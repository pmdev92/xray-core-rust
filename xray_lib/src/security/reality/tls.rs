...
pub struct RealitySecurityStream {
    server_name: String,
...
impl RealitySecurityStream {
    pub fn new(
        ...
        let connector = reality_tokio_rustls::TlsConnector::from(config);
        RealitySecurityStream {
            server_name,
            stream: Some(stream),
    ...
    async fn do_handshake(
        tls_connector: reality_tokio_rustls::TlsConnector,
        stream: Box<dyn AsyncXrayTcpStream + Send + Sync>,
        server_name: String,
        data: Vec<u8>,
    ) -> Result<
        (
            usize,
            reality_tokio_rustls::client::TlsStream<Box<dyn AsyncXrayTcpStream + Send + Sync>>,
        ),
        io::Error,
    > {
...
impl AsyncRead for RealitySecurityStream {
    fn poll_read(
        ...
        if let Some(conn) = self.connection.as_mut() {
            let result = std::task::ready!(Pin::new(conn).poll_read(cx, buf));
            return match result {
                Ok(_) => {
...
impl AsyncWrite for RealitySecurityStream {
    fn poll_write(
        mut self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &[u8],
        ...
        loop {
            ...
            let result =
                std::task::ready!(Pin::new(conn).poll_write(cx, this.write_buffer.as_ref()));
            match result {
                Ok(len) => {
