...
const MAGIC_DOMAIN: &str = "sp.v2.udp-over-tcp.arpa";

pub(crate) struct ShadowSocksUdpOverTcpVersion2Stream {
    pub(crate) is_connect: bool,
    pub(crate) connect_packed_sent: bool,
...
impl AsyncRead for ShadowSocksUdpOverTcpVersion2Stream {
    fn poll_read(
...
impl AsyncWrite for ShadowSocksUdpOverTcpVersion2Stream {
    fn poll_write(
        mut self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &[u8],
        ...
        if !self.connect_packed_sent {
            let is_connect = if self.is_connect { 1u8 } else { 0 };
            new_data.put_u8(is_connect);
            new_data.extend_from_slice(&self.address);
...
impl AsyncXrayTcpStream for ShadowSocksUdpOverTcpVersion2Stream {}
