...
impl TrafficState {
    pub fn new(uuid: Arc<Vec<u8>>) -> Self {
        Self {
            uuid,
...
impl VisionStream {
    pub fn new(stream: Box<dyn XrayTransport>, id: Arc<Vec<u8>>) -> Self {
        Self {
            stream,
            traffic_state: TrafficState::new(id),
    ...
    pub fn as_raw_stream(&mut self) -> &mut Box<dyn AsyncXrayTcpStream> {
...
impl AsyncRead for VisionStream {
    fn poll_read(
        ...
        if this.direct_read {
            let any: &mut dyn Any = this.stream.deref_mut() as &mut dyn Any;
            let tls_stream = any
                .downcast_mut::<TcpTransportStream>()
                .expect("tls stream");
            let tcp_stream = tls_stream.get_raw_stream();
        ...
        loop {
            match &mut this.read_state {
                ReadState::Reading => {
                    ...
                    log::debug!("Vision reader reads {} bytes", read_buffer.filled().len());
                ...
                ReadState::Unpadding(filled_size) => {
                        ...
                        || this.traffic_state.number_of_packet_to_filter > 0
                    {
                            ...
                            || this.traffic_state.padding_command == PaddingCommand::Continue
                        {
                            ...
                            log::debug!("Enable direct copy for reader",);
                        } else {
                            log::error!(
                ...
                this.write_buffer = if this.traffic_state.need_padding {
                        ...
                        && *TLS_APPLICATOIN_DATA_PREFIX == buf[0..3]
                    {
                        ...
                        if this.traffic_state.enable_xtls {
                            ...
                            log::debug!("Enable direct copy for writer",);
                        }
...
impl AsyncXrayTcpStream for VisionStream {}

impl XrayTransport for VisionStream {}
...
fn xtls_padding(
    ...
    let mut padding_len = if content_len < 900 && long_padding {
        let l: u32 = rand::thread_rng().gen_range(0..500);
        l + 900 - content_len
    } else {
    ...
    if !traffic_state.is_uuid_written {
        buf.put_slice(traffic_state.uuid.as_slice());
...
fn xtls_unpadding(bytes: &mut BytesMut, traffic_state: &mut TrafficState) -> Option<BytesMut> {
    if traffic_state.remaining_content == 0 && traffic_state.remaining_padding == 0 {
        if traffic_state.padding_command == PaddingCommand::Unknown(255) {
            if bytes.len() >= 21 && bytes[0..16] == *traffic_state.uuid.as_slice() {
                bytes.advance(16);
            } else {
                log::warn!("Xtls Unpadding ignored packet {}", bytes.len());
                return None;
            }
        }
...
fn xtls_filter_tls(bytes: &[u8], traffic_state: &mut TrafficState, src: &str) {
    ...
    if bytes.len() > 6 {
        ...
        {
            log::debug!("XtlsFilterTls found tls server hello! from {}", src);
        ...
        {
            traffic_state.is_tls = true;
            log::debug!("XtlsFilterTls found tls client hello! from {}", src);
        }
    }
    if traffic_state.remaining_server_hello > 0 {
        ...
        if find_str_in_str(hello_bytes, TLS_13_SUPPORTED_VERSIONS) {
            ...
            log::debug!("XtlsFilterTls found tls 1.2!");
            traffic_state.number_of_packet_to_filter = 0;
            return;
        }
        log::debug!(
    ...
    if traffic_state.number_of_packet_to_filter == 0 {
        log::debug!("XtlsFilterTls stop filtering {}", src);
    }
}
